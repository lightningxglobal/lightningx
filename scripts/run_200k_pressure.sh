#!/usr/bin/env bash
# 200K WebSocket connection pressure test.
# 8 desk-servers × 25K connections each.
# Works on macOS (lo0 aliases, psql, /tmp/aeron) and Linux (lo aliases,
# docker-postgres, /dev/shm/aeron, taskset aeronmd).
#
# Required loopback aliases for the default 16 source IP pool:
#   macOS: for i in $(seq 2 17); do sudo ifconfig lo0 alias 127.0.0.$i up; done
#   Linux: for i in $(seq 2 17); do sudo ip addr add 127.0.0.$i/8 dev lo; done
#
# No-forward: COUNTER_SHARD_COUNT=4 hardcoded in counter_shard.rs.
# With 8 desks the counter shards repeat: desk 0/4 share shard 0, desk 1/5
# share shard 1, etc.  pressure-i connects to desk-i using owner_shard = i%4
# and user_offset = (i/4)*conns_per_desk so each pressure process uses a
# distinct slice of the CSV — desk never forwards.
set -euo pipefail
...
TOKENS_CSV="${TOKENS_CSV:-$HOME/work/pressure_tokens/pressure_users_200k.csv}"
...
sleep "${DESK_WARMUP_S:-10}"

# ── pressure clients ──────────────────────────────────────────────────────
# Counter-forward layout with 8 desks and COUNTER_SHARD_COUNT=4:
#   desk 0 and 4 → counter_shard 0  (uid%4==0 users)
#   desk 1 and 5 → counter_shard 1  (uid%4==1 users)
#   desk 2 and 6 → counter_shard 2  (uid%4==2 users)
#   desk 3 and 7 → counter_shard 3  (uid%4==3 users)
# Each pressure-i uses owner_shard=i%4, user_offset=(i/4)*conns_per_desk
# so it selects a distinct slice of the CSV.  No forwarding occurs.
...
for ((i = 0; i < DESK_COUNT; i++)); do
  ...
  env PRESSURE_TOKENS_CSV="$TOKENS_CSV" \
    ...
    PRESSURE_WORKERS="${PRESSURE_WORKERS:-4}" NOFILE_LIMIT=262144 \
