...
fn main() {
    ...
    let buy_limit = Order::new(1, Side::Buy, 49_500, 10, TimeInForce::GTC, 0);
    let _ = engine.place_order(buy_limit).unwrap();

    println!("  2. 卖市价 7 个数量（应该以 49500 成交）");
    let sell_market = Order::new_market(2, Side::Sell, 7, 0);
    ...
    let sell_limit = Order::new(1, Side::Sell, 50_100, 4, TimeInForce::GTC, 0);
    let _ = engine.place_order(sell_limit).unwrap();
