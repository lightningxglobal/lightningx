...
if $IS_LINUX; then
  ...
  # send threads — one isolated physical core each (isolcpus=0-11, excl. 0=aeronmd 2=engine)
...
if $IS_LINUX; then
  ...
  env DATABASE_URL="$DATABASE_URL" AERON_DIR="$AERON_DIR" SYMBOLS=BTC_USDT \
    RUST_LOG="$ENGINE_RUST_LOG" TRACER_ENABLED=0 ENGINE_IDLE_SPINS=0 AERON_USE_HUGE_PAGES=1 \
...
for ((i = 0; i < DESK_COUNT; i++)); do
  if $IS_LINUX; then
    ...
    desk_env=(
      ...
      DESK_SPIN="${DESK_SPIN:-true}" DESK_SEND_CORE="$spin_cpu" AERON_USE_HUGE_PAGES=1
...
for ((i = 0; i < DESK_COUNT; i++)); do
  ...
  env PRESSURE_TOKENS_CSV="$TOKENS_CSV" PRESSURE_USERS="$conns" \
    PRESSURE_USER_OFFSET=0 PRESSURE_OWNER_SHARD="$owner_shard" PRESSURE_OWNER_SHARD_COUNT="${PRESSURE_OWNER_SHARD_COUNT:-$DESK_COUNT}" \
