...
// 400 bid + 400 ask levels pre-loaded; 5000 crossing GTC buys in batches of 20.
...
fn bench_deep_ob_batch() -> (f64, u64, u64) {
    ...
    // Pre-fill 400 bid + 400 ask levels (qty=100 each, matching old benchmark)
    for i in 0..400i64 {
        let _ = engine.place_order(make_order(i as u64, Side::Buy, base - i - 1, 100));
        let _ = engine.place_order(make_order(400 + i as u64, Side::Sell, base + i + 1, 100));
    }

    // 5000 crossing buys at base+500 in batches of 20 (each buys 1 lot, crosses best ask)
    let start = Instant::now();
    ...
    for _ in 0..(5000 / 20) {
        let mut batch: SmallVec<[Order; 20]> = SmallVec::new();
        for _ in 0..20 {
            batch.push(make_order(id, Side::Buy, base + 500, 1));
            id += 1;
        }
        let results = engine.match_orders_batch(batch).unwrap();
        total += results.len() as u64;
