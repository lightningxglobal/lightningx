#!/usr/bin/env bash
# 100K WebSocket connection pressure test.
...
#   macOS: for i in $(seq 2 11); do sudo ifconfig lo0 alias 127.0.0.$i up; done
#   Linux: for i in $(seq 2 11); do sudo ip addr add 127.0.0.$i/8 dev lo; done
set -euo pipefail
...
if $IS_LINUX; then
  ...
  # recv threads — isolated cores offset from send; all in 0-11 range
  CPU_DESK_RECV=(5 7 9 11 1 5 7 9)
  CPU_OTHERS="${CPU_OTHERS:-20-31}"      # tokio workers + gateway + pressure clients
else
  AERON_DIR="${AERON_DIR:-/tmp/aeron}"
fi
TOKENS_CSV="${TOKENS_CSV:-$HOME/work/pressure_tokens/pressure_users_100k.csv}"
...
# Verify only the IPs we actually need
...
for ((i = 0; i < DESK_COUNT; i++)); do
  ...
  env PRESSURE_TOKENS_CSV="$TOKENS_CSV" \
    ...
    PRESSURE_OWNER_SHARD="$i" PRESSURE_OWNER_SHARD_COUNT="${PRESSURE_OWNER_SHARD_COUNT:-$DESK_COUNT}" \
